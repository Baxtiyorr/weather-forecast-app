...
import { API_KEY, WEATHER_API } from "../../api/apis";

// get city data
export const getCityData = createAsyncThunk("city", async (obj) => {
  try {
    const request = await axios.get(
      `${WEATHER_API}/data/2.5/weather?q=${obj.city}&units=${obj.unit}&APPID=${API_KEY}`
    );
...
export const get5DaysForecast = createAsyncThunk("5days", async (obj) => {
  const request = await axios.get(
    `${WEATHER_API}/data/2.5/forecast?lat=${obj.lat}&lon=${obj.lon}&units=${obj.unit}&APPID=${API_KEY}`
  );
  const response = await request.data;
  return response;
});
